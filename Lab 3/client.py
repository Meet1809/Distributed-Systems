...
def getlogs():
    ...
    if latest_entry!=1 and message!="quit":
        ...
        msg = "logs,"+"dir"
        client_socket.send(bytes(msg,FORMAT))
        message_box.insert(tkinter.END,"Your Log operations are: ")

def undo(): #function to define UNDO operation from client side to server side 
    message = input_txt.get()
    if latest_entry!=1 and message!="quit":
        message_box.insert(tkinter.END, "Enter your username first")
        message_box.see(tkinter.END)
        ...
        msg = "undo,"+message
        client_socket.send(bytes(msg,FORMAT))
        # message_box.insert(tkinter.END,"UNDO done ")
...
if __name__ == "__main__":
    ...
    send_button = tkinter.Button(init_window, text="Enter username",width= 25, command=send)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="Select directory",width= 25, command=select)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="Create directory",width= 25, command=create)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="Delete directory",width= 25, command=delete)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="Move directory",width= 25, command=move)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="Rename directory",width= 25, command=rename)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="List all directories",width= 25, command=listall)
    send_button.pack()
    send_button = tkinter.Button(init_window, text="Get Logs ",width= 25, command=getlogs)
    send_button.pack()    
    send_button = tkinter.Button(init_window, text="UNDO : ",width= 25, command=undo)
    send_button.pack()    
    ...
    send_button.pack()
    send_button = tkinter.Button(init_window,text = "Sync",width = 25,command = sync)
    send_button.pack()
